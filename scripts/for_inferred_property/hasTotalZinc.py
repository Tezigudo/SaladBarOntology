...
# Constant for Zinc unit
ZINC_UNIT = "mg/100g"

# Constant for Zinc property
ZINC_PROPERTY = S.hasTotalZinc

def calculate_nutrient_for_salad(g, salad_name, nutrient_name="Zinc", unit=ZINC_UNIT, property=ZINC_PROPERTY):
    """
    Calculate total amount for Zinc for a given salad and update or create SaladSubstance instances.
    
    Args:
        g (Graph): The RDF graph to work with
        salad_name (str): Name of the salad instance (e.g., 'CapreseSalad')
        nutrient_name (str): Name of the nutrient to calculate (default: 'Zinc')
        unit (str): Unit for the nutrient (default: 'mg/100g')
        property (URIRef): Property for the nutrient (default: hasTotalZinc)
    """
...
def process_all_salads_for_nutrient(nutrient_name="Zinc", unit=ZINC_UNIT, property=ZINC_PROPERTY):
    """
    Retrieve all Salad instances and calculate their total for a specific nutrient (Zinc).
...
if __name__ == "__main__":
    process_all_salads_for_nutrient("Zinc", ZINC_UNIT, ZINC_PROPERTY)
