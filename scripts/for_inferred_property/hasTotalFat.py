...
# Constant for Fat unit
FAT_UNIT = "mg/100g"

# Constant for Fat property
FAT_PROPERTY = S.hasTotalFat

def calculate_nutrient_for_salad(g, salad_name, nutrient_name="Fat", unit=FAT_UNIT, property=FAT_PROPERTY):
    """
    Calculate total amount for Fat for a given salad and update or create SaladSubstance instances.
    
    Args:
        g (Graph): The RDF graph to work with
        salad_name (str): Name of the salad instance (e.g., 'CapreseSalad')
        nutrient_name (str): Name of the nutrient to calculate (default: 'Fat')
        unit (str): Unit for the nutrient (default: 'mg/100g')
        property (URIRef): Property for the nutrient (default: hasTotalFat)
    """
...
def process_all_salads_for_nutrient(nutrient_name="Fat", unit=FAT_UNIT, property=FAT_PROPERTY):
    """
    Retrieve all Salad instances and calculate their total for a specific nutrient (Fat).
...
if __name__ == "__main__":
    process_all_salads_for_nutrient("Fat", FAT_UNIT, FAT_PROPERTY)
